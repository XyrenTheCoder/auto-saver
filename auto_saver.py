...
class Main(tk.Tk):
    def __init__(self):
        ...
        self.interval = ("1 sec", "2 secs", "5 secs", "10 secs", "30 secs (recommanded)", "1 min", "2 mins", "5 mins", "10 mins", "30 mins") #time interval
        self.application = ("Visual Studio Code", "VSCodium", "Visual Studio", "PyCharm", "Use universal keybinds / Others")
        self.keybindset0 = ("Ctrl + S", "Other specified") #universal keybind
        self.keybindset1 = ("Ctrl + S", "Ctrl + Shift + S", "Ctrl + K, S", "Other specified") #windows keybind for vscode
        self.keybindset2 = ("Ctrl + S", "Ctrl + Shift + S", "Other specified") #linux keybind for vscode and visual studio
    ...
    def widgets(self):
        global label1, label2, label4, option_menu1, option_menu2, option_menu4, rowcount
        ...
        option_menu1.place(x=365, y=42.5) #40 + pady/2
        ...
        option_menu2.place(x=365, y=82.5) #(+40 inherited)
        ...
        option_menu4.place(x=365, y=122.5) #inherit last coordinates (+40)
    ...
    def specified1(self):
        ...
        if self.option_var1.get() == "Visual Studio Code" or self.option_var1.get() == "VSCodium" and os.name == 'nt':
            label4.place(x=25, y=122.5)
            option_menu4 = ttk.OptionMenu(self, self.option_var2, 'Unspecified', *self.keybindset1, command=self.option2_changed)
            option_menu4.place(x=365, y=122.5) #inherited coordinates (+40)
        elif self.option_var1.get() == "Visual Studio Code" or self.option_var1.get() == "VSCodium" and os.name == 'linux':
            label4.place(x=25, y=122.5)
    ...
    def specified2(self):
        ...
        if self.option_var2.get() == "Other specified":
            ...
            option_menu5.place(x=365, y=162.5) #inherit last coordinates (+40)
        else:
            try:
                label5.destroy()
                option_menu5.destroy()
    ...
    def start(self):
        ...
        time = self.option_var0.get()
        if time.endswith("(recommended)"):
            time.replace(" (recommended)", "")
        if time.split()[1] == 'secs':
            interval = int(time.split()[0])
        elif time.split()[1] == "mins":
            interval = int(time.split()[0]) * 60

        keys = self.option_var2.get()
        if keys == 'Other specified':
            keys = self.option_var5.get()
            keys = keys.replace(' ', '')
            if ',' in keys:
                subkeys, key2 = keys.split(',')
                if '+' in subkeys:
                    key0, key1 = subkeys.split('+')
            elif '+' in keys:
                subkeys = keys.split('+')
                if len(subkeys) > 2:
